...
export class CartStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const lambdaFunction = new lambdaNodejs.NodejsFunction(this, 'NestJsLambdaFunction', {
      ...
      environment: {
        PGHOST: dbInstance.dbInstanceEndpointAddress,
        PGUSER: dbCredentialsSecret.secretValueFromJson('username').toString(),
        PGPASSWORD: dbCredentialsSecret.secretValueFromJson('password').toString(),
        PGDATABASE: 'cartdb',
        PGPORT: '5432'
      },
      vpc,
    ...
    item.addMethod('GET');   // GET /items/{id}
    item.addMethod('PUT');   // PUT /items/{id}
    item.addMethod('DELETE');// DELETE /items/{id}
  }
}
